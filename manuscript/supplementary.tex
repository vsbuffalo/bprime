...
\subsection{Out-Sample Error Estimation and $R_\text{LOO}^2$}

We fit each of our B' model fits 22 times, excluding a whole chromosome each
time to calculate out-sample prediction error. Each fit used 10,000 random
starting positions as the main fits. The out-sample prediction error for all
bins in chromosome $C$ left out is,

\begin{align}
    SSE(C) = \sum_{b \in C} (\widehat{\pi}_{b,Y\not \in C}  - \pi_b)^2
\end{align}
%

where $\widehat{\pi}_{b,Y\not \in C}$ denotes the predictions for window $b$
fit on data excluding chromosome $C$. The average out-sample error $SSE$ across
chromosomes is then the average of $SSE(C)$ across chromosomes, weighted by the
number of bins per chromosome. We then calculate the out-sample coefficient of
determination $R_\text{LOO}^2$

\begin{align}
    R_\text{LOO}^2 = \frac{SSE}{}
\end{align}
