...
\section{The Background Selection Models}

Under the background selection model, the diversity in window $b$ is $\bar{\pi}(b,
...
BGS models assume multiplicative fitness effects across sites. Thus, the total
reduction due to segments across the genome is the product of the individual
reductions, 
...
mutation rates are on a fixed $z$-element grid, which is interpolated over
during ML optimization. The number of annotation features $K$ is variable and
set by the annotation data specified. There are two ways to parameterize the
mutation rates and DFE for all annotation classes in background selection
models. First, for the \emph{free-mutation} parameterization, each annotation
feature class has a free mutation rate parameter for each of these selection
coefficients (as in equation \eqref{suppeqn:b}).
...
\subsection{Calculation of the classic BGS B Maps}

The classic BGS model expresses the reduction for a focal site in the middle of
a segment \parencite{Hudson1995-xc,Hudson1994-oh,Nordborg1996-nq}; here we
consider a focal site directly to an arbitrary side of a segment under a
deleterious mutations, and extend the model to handle a focal segment
arbitrarily far from the segment. Using our notation from equation
...
2 \mu L$ and $R = r_\text{BP} L$ are the segment-wide mutation diploid mutation
