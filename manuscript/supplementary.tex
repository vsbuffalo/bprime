...
\section{The Generalized Background Selection Models}

Under the generalized background selection models used in our likelihood, the
diversity in window $b$ is $\bar{\pi}(b, \Psi) = \bar{B}(b, \Psi) \pi_0$. Here,
$\bar{B}(b | \Psi)$ is the predicted reduction in diversity due to BGS in
window $b$, given background selection parameters $\Psi$. In practice, we
pre-calculate $B(x | \Psi)$ at fixed sites $x$ across the genome, and take the
average of the fixed sites $B$ values within widow $b$ for the average
$\bar{B}(b | \Psi)$. 

GBGS models assume multiplicative fitness effects across sites. Thus, the total
...
the segment. Using our notation from equation \eqref{suppeqn:b}, the classic
...
our derivation). We describe how we use the models here.





\subsection{Discretization of Parameters}
