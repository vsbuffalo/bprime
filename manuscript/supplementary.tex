...
Thus $\mathbf{P}_l = [n_S, n_D]$, and the pairwise diversity is calculated as

\begin{align}
  \bar{\pi}(l) = \frac{n_D}{n_T}
\end{align}

which is identical to the more common expression of this estimator, 

\begin{align}
  \bar{\pi}(l) = \frac{2}{n(n-1)} \sum_{i < j}^n k_{i,j}
\end{align}
...
\begin{align*}
  \bar{\pi}(\mathcal{A}) = \frac{\sum_{l \in \mathcal{A}} \bar{\pi}(l)}{L_\mathcal{A}}.
\end{align*}

where $L_\mathcal{A} = |\mathcal{A}|$ is the number of accessible bases. Note
that both $L_\mathcal{A}$ and the sum are random variables in genomic data.
The denominator varies along the genome, based on the number of
reliably-genotyped and accessible bases in the reference genome. This is a
ratio estimator, and is thus biased, since by Jensen's inequality
$\E(\nicefrac{y}{x}) \ge \nicefrac{\E(y)}{\E(x)}$.

We explore this with block-bootstrap bias correction. TODO
