...
associations with fitness backgrounds, the non-heritable
...
frequency change across generations due to linked polygenic selection. Incorporating the impact
of time-invariant autocorrelation created by linked selection as a rescaled rate of drift
...
time under weak polygenic linked selection. Others have found that the genealogies under purifying
selection can be characterized by such a time-dependent effective population
size \parencite{Nicolaisen2013-gv}.


\subsection{The Fitness-Effective Population Size}

Now, we consider how to turn this expression for the variance in neutral allele
frequency at time $t$ under weak linked selection into an expression for the
\emph{fitness-effective} population size analogous to drift-effective population
...
at time $t$ (which we here are calling fitness-effective population size,
$N_{d,t}$),
...
In many cases, we care about the asymptotic $N_f$, 

\begin{align}
    N_f := \lim_{t \to \infty} N_{d,t} \approx \frac{4N}{4 V_A Q_\infty^2 +V_n+2}.
\end{align}
%
Now, we turn to different expressions for $Q_t^2$ and $V_A$ under different
types of polygenic selection.

\subsection{The Fitness-Effective Population Size Under Polygenic Selection}
...
more elaborate breeding structures \parencite{Wray1990-zf,Woolliams1993-qo}.
...
\subsection{The Fitness-Effective Population Size Under Strong Background Selection}
...
\subsection{The Fitness-Effective Population Size Under Weak and Strong Negative Selection}
...
Ratchet in assexual populations, the rate $R$ of fixation of deleterious mutations per generation per
...
background selection, $V_A = Us = V_\text{BGS}$ (equation \ref{supp-eq:va_bgs}).
...
asymptotic reduction in fitness-effective population size uses the $Q_\infty^2$ and is
...
selection models that predict the fitness-effective population size under
polygenic selection and linkage. Our work here only considers the contribution
of deleterious mutations to fitness variation. In this
...
\subsection{Models of Linked Selection}
...
non-linear equations (Equation \ref{eq:main_eqns} in the main paper) to calculate $b$ at each segment, these are pre-computed
...
inflation factor $Q_t^2$ that sets the fitness-effective population size varies
...
pairwise coalescence rate per generation, which is not constant under linked selection. 
...
Non-polymorphic sites have $n_S = \binom{n}{2}$ and $n_D = 0$, and missing
data has $n_S = n_D = 0$.
...
per-basepair genotype calls (gVCF) produced by Illumina's DRAGEN pipeline for the high-coverage 1000 Genome samples
\parencite{Illumina_Inc2020-dk,Byrska-Bishop2022-tn}. The original samples were sequenced to 30x by
...
The gVCFs were filtered using a custom Python tool, \texttt{gvcf2counts.py} (available in the repository associated with the manuscript at
...
\path{Homo_sapiens.GRCh38.107.chr.gff3.gz}]{Cunningham2022-vk} and PhastCons
regions \path{phastConsElements100way.bed.gz}]{Siepel2005-wh}. Then, we remove
200 basepairs of buffer around each of these regions (i.e. \texttt{bedtools
...
insensitive to the buffer width, but increasing slop widths around putatively
...
\subsection{Table of \texorpdfstring{$R^2$}{R2} Values}
\label{supp:tbl-r2}
\begin{table}
\centering
...
\subsection{Table of \texorpdfstring{$R^2$}{R2} Values (Strong Selection Grid)}
...
\caption{$R^2$ and mutation rate estimates for all primary models fit with the
    ...
    the main analysis notebook in the GitHub repository for more information. $R_\mathrm{IS}^2$ is the in-sample $R^2$, and $R_\mathrm{LOCO}^2$ is the leave-one-chromosome-out $R^2$. }
...
\subsection{Derivation of Approximate Coalescence \texorpdfstring{$R_\text{coal}^2$}{R2}}
\label{supp:r2-coal}
...
(Supplementary Figure \ref{suppfig:drift_r2}B). 
...
% \begin{figure}[htbp] \centering
%     \includegraphics[width=0.5\textwidth]{figures/supplementary/subfigure_R2_scale.pdf}

%     \caption{How the window size effects our theoretic $R_\text{coal}^2$ versus
%         our CADD 6\% models classic BGS (B) and new B' methods at two scales (1
%         Mbp and 100 kbp). Additionally, we have overlaid the CADD 6\%
%         out-sample $R_{OS}^2$ estimates across many window sizes from
%     \textcite{Murphy2022-sj}. We note that the theoretic $R_\text{coal}^2$
% quickly declines to zero since the coalescence and mutational noise quickly
% begins the linked selection signal.}


%   \label{suppfig:coal_r2_scale}
% \end{figure}
...
under some evolutionary model. We only consider  models of purifying selection here, so the mean
function for position $x$ is the product of the proportion by which selection reduces
...
$\Phi$ is the set of parameters (i.e. the DFE matrix $\mathbf{W}$ and
...
the upper bound for the PhastCons Priority model; for these fits we increased this first to $\mu = 8 \times
10^{-8}$ and then again to $\mu = \times 10^{-7}$, find it still always reached the upper bound. This did not occur for CADD 6\% locally rescaled models.
...
pickle files in the Data Dryad repository (\url{https://doi.org/10.5061/dryad.qnk98sfnv}). In one case, our optimization
...
\subsection{Out-Sample Error Estimation and \texorpdfstring{$\widehat{R}_\text{LOCO}^2$}{R2}}

To estimate out-sample prediction error, we fit each model leaving one
chromosome out (LOCO) and calculating the out-of-sample prediction error on
...
\begin{figure}[htbp]
  ...
  \caption{An optimization diagnostic plot showing an example of the bimodal likelihood optima, or a numeric optimization issue. The first four rows are the DFE estimates, where color indicates the parameter
  value (brighter colors closer to 1, darker colors closer to 0). 
  The $\pi_0$ and $\mu$ estimates are shown in the bottom two 
  rows. Note that different parameter combinations co-occur, 
  indicating in the top 100 optima, there is an apparent other
  mode that varies with mutation rate.}
...
a numeric optimization issue or the presence of multiple local optima. Given that this looked
...
\parencite{International_HapMap_Consortium2007-nu}, PhastCons regions \parencite{Siepel2005-wh}, and UTRs
...
$\var(\pi) \approx \nicefrac{2\theta^2}{9}$ in an infinite sample at a non-recombining locus. One can
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{figures/supplementary/rescale_pi_pred_plot.pdf}

  \caption{Chromosome 2 predictions on the YRI samples, with locally rescaled
      model fits. The observed data is the dark gray line, and the normal MLE for
      ...
      using the standard B' map (without local rescaling) and the maximum likelihood
...
strong selection $s=10^{-3}$ ($2N_e s = 20$), local rescaling does not change
...
\begin{figure}[htbp]
  ...
  \caption{The DFE for YRI samples with local rescaling. The maximum
...
Below are the model fit results for locally rescaled model runs, with the upper mutation rate boundary set to $\mu = 8 \times 10^{-8}$. Each model fit is the direct output from a
\texttt{bgspy}. The \texttt{w grid} and \texttt{t grid} are the grids the B and B' maps are built on; note that the mutation grid \texttt{w} is an exponential that is inferred later, so optimization is done continuously over this. Each model output shows the maximum likelihood estimates, with uncertainty where available. 
...
SimplexModel (interpolated w): 6 x 7 x 3
  ...
  pi  = 0.000752601
  mut. rate  = 8e-08 
  Ne = 26,572 (if mu=1e-8), Ne = 13,286 (if mu=2e-8)
...
SimplexModel (interpolated w): 6 x 7 x 3
  ...
  pi  = 0.00106447
  mut. rate  = 8e-08 
  Ne = 36,974 (if mu=1e-8), Ne = 18,487 (if mu=2e-8)
...
SimplexModel (interpolated w): 6 x 7 x 3
  ...
  pi  = 0.000801495
  mut. rate  = 8e-08 
  Ne = 27,755 (if mu=1e-8), Ne = 13,877 (if mu=2e-8)
...
pi  = 0.000752601
mut. rate  = 7.291e-08
Ne = 25,506 (if mu=1e-8), Ne = 12,753 (if mu=2e-8)
R² = 60.6057% (in-sample)
W =
          cadd6
------  -------
...
pi0  = 0.000801495
mut. rate  = 5.991e-08
Ne = 26,856 (if mu=1e-8), Ne = 13,428 (if mu=2e-8)
R² = 62.0668% (in-sample)
W =
          cadd6
------  -------
...
pi  = 0.00106447
mut. rate  = 6.024e-08
Ne = 35,914 (if mu=1e-8), Ne = 17,957 (if mu=2e-8)
R² = 68.6747% (in-sample)
W =
          cadd6
------  -------
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{figures/supplementary/feature_priority_pop_dfe_strongselgrid.pdf}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{figures/supplementary/phastcons_priority_pop_dfe_strongselgrid.pdf}
...
\begin{figure}[!htb]
  ...
  \caption{Blue lines represent the residual between predicted an observed diversity levels along chromosomes. The orange line is a lowess curve, with bandwidth chosen by cross-validation.}
