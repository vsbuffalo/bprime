...
\section*{Approximate $R^2$ Limit}

We derive a rough approximation of the theoretic upper bound of $R^2$ by
noting that,

\begin{align}
    R^2 &= \frac{SS_\text{reg}}{SS_\text{reg} + SS_\text{res}} \\ 
        &= \frac{\sum_i (\pi_i - \widehat{\pi}_i)^2}{\sum_i (\pi_i - \widehat{\pi}_i)^2 + SS_\text{res}}
\end{align}

where $\pi_i$ is the observed diversity in window $i$, and $\widehat{\pi}_i$ is
the predicted diversity under a particular model.

Then, $SS_\text{res}$ is the residual noise due to the randomness in mutation
and average coalescence times in the window. If we make the assumptions that
mutation rates are heterogeneous across the genome and the coalescence rate in
a window is simply a background rate rescaled by the reduction factor $B_i$,
then,

\begin{align}
    SS_\text{res} &= \frac{1}{n} \sum_{i=1}^n \var(w\pi_i) \\ 
                  &= \frac{1}{n} \sum_{i=1}^n \var(w\pi_0 B_i) 
\end{align}

where $w$ is the width of the window such that the per-basepair $\pi$ is
appropriately scaled. Then, we note that the variance in pairwise diversity is
equivalent to the variance in the number of segregating sites for a sample of 
two, $\var(\pi) = \var(S_2)$. From \textcite[eq. 7.20]{Wakeley2009-ua}
...
wanted to check the sensitivity of this calculation to this.
...
(Figure \ref{suppfig:r2_upper} We note, however, that this rough approximation
assumes constant demography, and that the out-of-Africa bottleneck causes a
reduction in effective population size that would shrink the variance of
exponential coalescence times, but a bottleneck could in fact inflate these
variances. A more accurate approximate of the upper $R^2$ bound could rely on
simulations with more realistic demography.

\begin{figure}[htbp] \centering
    \includegraphics[width=0.5\textwidth]{figures/supplementary/suppfigure_R2_upper.pdf}
