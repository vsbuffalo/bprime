...
process. We use the metric $R_\text{LOO}^2$, which is the proportion of the
observed variance in genomic diversity at the megabase scale predicted by our
model. We experimented with a few smaller spatial scales, but our results were
consistent with previous results suggesting the human linked selection signal
...
With multiple features classes and a map $k(g)$ between segments $g$ to their
...
Here, $\mu$ is the mutation rate and $W_{j,k(g)}$ is a DFE entry in the
$\mathbf{W}$ is the $n_s \times K$ DFE matrix over the selection grid and
features. Since each column of the DFE $\mathbf{W}$ must sum to one, numeric
optimization was done in unconstrained $n_s-1$ dimensional softmax space. Our
binomial likelihood modeled the number of different pairwise comparisons
observed per window given the predicted pairwise diversity and total number of
comparisons implied per window (see Supplementary Materials Sections
\ref{supp:data-summary} and \ref{supp:likelihood}).
...
random starts (see Supplementary Materials Section \ref{suppsec:optima}). We
assessed model fit using out-sample predictive error, calculated by leaving out
a whole chromosome during fitting and predicting its diversity. To calculate
uncertainty, we used a block jackknife approach in 10 Mbp windows
(Supplementary Section \ref{supp:jackknife}).
...
We use two types of forward simulations in this paper; both use SLiM version
4.0.1 \parencite{Haller2023-uk,Haller2019-vu}. First, we simulated a 100 kbp
segment in a population of $N=1000$ diploids across grids of mutation and
selection parameters to assess the accuracy of the SC16 theory. Each simulation
recorded the number of fixations and the fitness variance per generation, and
at the end $10N$ generations, the ancestral recombination graph was recorded so
downstream Python scripts could calculate diversity. Each of these segment
simulations was replicated $10,000$ times and averaged to compare against
theoretic expectations.

Second, we conducted forward simulations of negative selection on whole human
chromosomes. These simulations used the HapMap recombination map
\parencite{International_HapMap_Consortium2007-nu} and treated coding,
PhastCons, and UTR regions as an approximate set of conserved regions. We used
these whole-chromosome simulations for three purposes. First, we simulated
negative selection on chromosome 10 to confirm our method to estimate classic B
and new B' maps matched the average empirical reduction map across a grid of
mutation and selection parameters. Second, we evaluated our composite
likelihood method by simulating negative selection on the first five human
chromosomes, across grids of mutation and selection parameters. Multiple
independently-evolved chromosomes were combined into a synthetic genome in
order to have enough data for our procedure. Third, we conducted additional
simulations like these, but varying the dominance coefficient and expanding the
population by a factor of 1.004 each generation, starting $9.3$ generations in
to mimic the human expansion out-of-Africa (XXX). More details about these
simulations can be found in Supplementary Section \ref{supp:simulation}.
