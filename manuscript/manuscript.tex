...
Along the chromosome, our approach models the observed megabase-scale diversity
in window $i$ as the predicted level under our negative selection model,
$B_i'$, plus some residual error $\varepsilon_i$, $\pi_i = B_i' +
\varepsilon_i$. The out-sample $R_\text{LOO}^2$ estimates the ratio of variance
explained by the negative selection model ($\var_i(\pi_i)$) to the total
variance in diversity along the genome ($\var_i(\pi_i) = \var_i(B_i) +
\var_i(\varepsilon_i) + 2\cov_i(B_i', \varepsilon_i)$, \cite{Murphy2022-sj}).
...
and $\var_i(\varepsilon_i)$ was determined by the variance in coalescence times
due to drift alone with an effective population size rescaled by $B_i'$. We
calculate this ``drift $R^2$" by calculating the theoretic coalescence time
variance in a megabase window with recombination and an effective population
size of $B_i' N_e$ where we use our best-fitting model's as a plug-in estimator
of $B_i'$. We note that this is a rough approximation, since this calculation
assumes constant demography and that the variance in coalescence times under
negative selection is likely lower than expected under drift alone. Still, we
find that our out-sample $R_\text{LOO}^2$ for Yoruba individuals
($R_\text{LOO}^2 \approx 68$\%) is close to the theoretic drift
$R_\text{drift}^2 \approx 67$\%. For bottlenecked out-of-Africa populations,
$R_\text{drift}^2 \approx 64$\% compared to the observed $R_\text{LOO}^2
\approx 62$\% for CEU and $R_\text{LOO}^2 \approx 59$\% for CHB. We note that
because bottlenecks increase the variance in coalescence times beyond the level
implied by the effective population size, the disagreement between observed
$R_\text{LOO}^2$ and the theoretic $R_\text{drift}^2$ is likely to shrink.
Overall, this suggests that negative selection models explain the vast majority
coalescence time variation at the megabase-scale that is capable of being
explained (i.e. that is not coalescence noise).
