...
selection processes: hitchhiking (linked selection due to beneficial alleles
increasing in frequency) and background selection (linked selection due to the
selection against deleterious alleles). In turn, statistical methods have use
this theory for a variety of wide-ranging empirical problems. Early on,
researchers found correlations between recombination rates and genetic
diversity in organisms with limited recombination like \emph{Drosophila} (XXX),
consistent with linked selection processes acting on genome-wide scales. With
population genomic data, evidence of small-scale (XXX kb) dips in genetic
diversity was found, caused by rare but strong selective sweeps in humans (XXX)
and other species (XXX). Remarkably, a simple mathematical model of background
selection could closely predict levels of human-chimp divergence at the
megabase scale (XXX) from genome-wide patterns of recombination and regions
conserved between species (McVicker). Another central empirical goal is to
disentangle the relative effect of the these two selection processes on levels
of genetic diversity (Eyalshiv).  

Additionally, linked selection can impact other evolutionary processes in two
critical ways. First, while most evolutionary models consider genetic drift to
be the main source of random fluctuations in neutral allele frequencies, linked
selection has a similar impact on neutral sites, increasing the variance in
allele frequency change as if the population size were effectively smaller.
Second, by perturbing the frequencies of neighboring alleles, selection can
interfere with their selection dynamics, known as Hill-Robertson interference
(XXX). 
...
This is known as selective interes
