...
\begin{align}
  ...
  Q_t &= 1 + \sum_{i=1}^t \left((1-r)\kappa\right)^i \\
  Q_\infty &= \frac{1}{1-(1-r) \kappa}
\end{align}
...
(c.f. \cite{Santiago1998-bs} equation 4b). Santiago and Caballero ignore the
covariances between the locus-specific variation decay rates $1-\kappa_l$ terms
and the locus-specific variances, $V_{h,l}$. This allows for the terms to be
separated,

\begin{align}
  N_{e,t} &\approx N \exp\left(- \frac{V_h}{2} \sum_{l=1}^L Q_{t,l}^2\right).
\end{align}
...
implementation of the classic BGS model, XXX).

If we assume that the $L$ selected sites are evenly distributed in a region of
length $G$ basepairs, we can rewrite the sum as $\nicefrac{L}{G} \sum_{l=1}^G
Q_{t,l}^2$
...
For a continuous recombination map, the recombination fraction between the
neutral allele and a selected site $d$ map distance away is given by Haldane's
map function, $r = (1-e^{-2d})/2$. Santiago and Caballero assume the focal
neutral site is in the middle of the chromosome, and integrate over the map
distance. Assuming that linkage is not too tight, 

\begin{align}
  Q_\infty &= \frac{1}{1-r \kappa}
\end{align}
...
For our model, we consider the contribution of individual contiguous segments
of basepairs rather than the whole genome. 

This summation can then in turn be
...
expected number of mutations across individuals is $\E_i(n_i) =
\nicefrac{\mu}{s}L$ where $L$ is the total number of basepairs that can be
mutated (the mutational target size).  Under a multiplicative fitness function
in an infinite population, selection cannot generate linkage disequilibria
...
\begin{align}
  V_\text{BGS} &:= \V_i(w_i) = 2 \sum_{l=1}^L p(1-p) s^2 \\
            &= 2 L \frac{\mu}{s} \left( 1 - \frac{\mu}{s}\right) s^2 = U s + \mathcal{O}(\mu^2) \approx Us.
\end{align}

where $U$ is defined $U := 2 L \mu$, the deleterious mutation rate per diploid
genome, per generation. They then set $V_A = V_\text{BGS}$, and note that $V_m
\approx Us^2$, since mutations have frequency of $\nicefrac{1}{2N}$ and each
increase the variance by a factor of $s^2$. Note that because the classic BGS
model assumes deterministic selection dynamics (i.e. an infinite population),
the equilibrium $Z$ given by equation \eqref{eq:Z} excludes the reduction due
to drift, which reduces the variance by an additional factor $1 -
\nicefrac{1}{2N_e}$. Thus, the reduction factor under the strong BGS model is,

\begin{align}
  Z \left(1 - \frac{1}{2N_e}\right) = (1 - s)\left(1 - \frac{1}{2N_e}\right) \approx 1 - s -\frac{1}{2N_e}
\end{align}
...
the reduction under the classic BGS model is,

\begin{align}
  N_e = N \exp()
\end{align}

\subsection*{Weak Selection Dynamics}
...
lost due to drift. The approach of \textcite{Santiago2016-mu} that we implement
in our methods is to derive an equation for 
