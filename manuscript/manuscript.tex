...
statistical methods that fit genome-wide diversity to linked selection
processes rely on the classic BGS model to estimate the distribution of fitness
effects and rate of deleterious mutations, it is important to characterize and
remedy these shortcomings. However, extending background selection theory to
model weak negative selection is challenging, and it is worthwhile to develop
some intuition for how the classic background selection works and why this is
the case. 

The classic background selection model \parencite{Charlesworth1993-gb} 

First, the frequency trajectory of a weakly selected allele is
predominantly determined by stochastic evolutionary processes. When the changes
due to stochasticity are large, deterministic approximations that work well
under strong selection no longer capture selection dynamics. Second, weakly
selected alleles are impacted by two sources of evolutionary stochasticity:
genetic drift (i.e. non-heritable differences in offspring number and Mendelian
segregation) and genetic draft, the stochastic changes in allele frequency
caused by selection on the same genetic background
\parencite{Neher2013-dz,McVean2000-bt}. 

Moreover, when the magnitude of stochastic changes can exceed the deterministic
changes caused by selection, mutations that decrease fitness now have a chance
of fixation \parencite{Kimura1962-su,Malecot1948-zv}. Because there is a steady
influx of weakly deleterious mutations, each with a non-zero chance of
substitution, there must be a corresponding substitution process. The rate of
this deleterious substitution process (e.g. ``Muller's ratchet") is 


By contrast, when selection is assumed to be strong,
deterministic models of mutation and selection that ignore these sources of
stochasticity adequately capture the dynamics. Since weakly deleterious
mutations have some chance of fixation, 
