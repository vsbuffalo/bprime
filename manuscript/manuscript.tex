...
average selection coefficient across CADD 6\% regions of $\wisehat{\bar{s}}
\approx 0.038$.
...
constrained grid models fit the data less well than the default grid. On
average, the constrained grid fits had $R_\text{LOCO}^2$ one percentage point
lower (see Supplementary Table \ref{supp:tbl-r2}). As expected, the constrained
grid models distributed mass onto the next strongest selection coefficient,
$s=10^{-2}$. While the DFE could be truly bimodal for highly-conserved regions
with a very high average selection coefficient, a likely alternative is that
these results reflect a non-identifiability issue between selective effects of
