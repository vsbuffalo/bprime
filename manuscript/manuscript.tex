...
(see Supplementary Table \ref{supp:tbl-r2}). 

Our method's estimates for the DFE rely on a
pre-specified logarithmic grid; we use a fairly fine-scale grid from $10^{-8}$
to $10^{-1}$. 

Across the CADD models, we note a discontinuity in DFE estimates,
where the largest mass is placed on the strongest selection coefficient ($s =
10^{-1}$), very little on the next coefficient ($s=10^{-2}$), and mo($\approx
41\%-67\%$)

These DFE estimates also suggest that our method
can accurately estimate when regions are under weak or no constraint. 

Across our full-coverage models, the estimated DFE for the ``other" feature
containing the rest of the genome is estimated to nearly 100\% neutral
($s=10^{-8}$). In some cases (e.g. the CADD 6\% and 8\% models) the ``other"
estimates a small fraction ($\approx 3\%-5\%$) of mutations in the ``other"
region have a strongly deleterious impact ($s=10^{-1}$). This is likely due to
non-identifiability between $\pi_0$ and a small chance of deleterious mutations
occurring on the unannotated remainder of the genome.

Our DFE estimates 

Thus, even though our model can better distinguish
weak versus strong selection, our DFE estimates suggest 
