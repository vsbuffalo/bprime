...
variation $C^2$ (on a relative fitness scale, i.e. the squared coefficient of
variance) becomes inflated by a factor $Q^2$. Intuitively, the product $V^2
Q^2$ represents the expected total variance a neutral mutation experiences over
its lifetime in a system with draft. This \emph{heritable} fitness variance
that neutral allele experiences can then be included in Equation
\eqref{eq:simple_Ne} (assuming the usual Wright--Fisher non-heritable variance,
i.e. $V^2=2$),
...
term $Q^2$ has a temporal dimension. For notational efficiency, we assume $Q^2$
refers to the asymptotic $Q_t^2$ as $t \to \infty$ and $N_e$ is the
\emph{asymptotic} draft effective population size. In general, however,
...
(which is approximately heterozygosity in most cases) depends on the pairwise
coalescence rate per generation, which will is not constant under draft. The
benefit of using a forward-time model of draft is that it the inflation factor
is invariant with respect to the particular fitness background the neutral
allele becomes stochastically associated with. By contrast, the difficulty with
backwards-time models is that the coalescence rates experienced by a lineage
are not invariant to which lineage was sampled.

Equation \eqref{eq:main_Ne} 


Shortly, we will show that Robertson (1961) and Santiago and Caballero's (1995,
1998, 2016) approach 
