...
beget many descendents themselves (and likewise with small families). However,
the same process acts at the genomic level, as the perturbations to a neutral
allele's trajectory from its particular fitness background tend to occur in the
same direction across generations. The additional heritable variation in
...
the impact of time-invariant autocorrelation created by draft as rescaled rate
of drift is analogous to using the Green--Kubo relation to find the transport
coefficient \parencite{Green1954-kl,Kubo1957-va}.
...
variation (i.e. $V = 2 + 4 Q^2 V_A$),

\begin{align}
    \label{eq:main_Ne}
    N_e = \frac{N}{Q^2 V_A + 1}
\end{align}

(c.f. \cite{Robertson1961-ho,Santiago1995-hx}; see XXX for a proof). Note that
because the variance under drift increases each generation due to the
accumulation of drift perturbations and draft autocorrelations, the inflation
term $Q^2$ has a temporal dimension. For notational efficiency, we let $Q^2$
refer to the asymptotic $Q_t^2$ as $t \to \infty$ and $N_e$ is the
\emph{asymptotic} weak draft effective population size. In general, however,
statistics such as heterozygosity depend on the cumulative autocorrelation
factor \emph{up to some time} $t$, $Q_t^2$ \parencite[p.
...
for genome-wide negative selection processes, the asymptotic weak draft
effective population size experienced by an arbitrary neutral site under the
influence of all linked regions is,

\begin{align}
    \label{eq:polygenic_Ne}
    N_e \approx N \exp\left(-\sum_{i=1}^n V_{A,i} \frac{Q_i^2}{2}\right)
\end{align}
...
additive genetic fitness variance. The degree to which segment $i$ inflates the
total variance in reproductive success depends on the how quickly the
autocorrelations in 

autocorrelation function in
reproductive success. Assuming a constant selective environment, the
autocorrelation in reproductive success of the neutral allele is given by a
geometric autocorrelation function $C(t) = \[(1-r_i)(1-\kappa_i)\]^t$. The
decay in autocorrelation is due to 
...
Segment $i$ inflates the total variance in
reproductive success of the neutral allele by $Q_i^2$, according to two key
parameters. First, 

according to the chance it stays
associated each generation, given by the recombination fraction $r_i$, and the
extent that the associated heritable fitness variation within the segment
determines future reproductive success in later generations. Assuming a
constant environment for selection, this 


, according to
the recombination fraction $r_i$ to the segment and the extent to which the
fitness variation in the segment perturbs the trajectory in later generations. 

the fitness
variation associated with the neutral allele 

persistence of the
associated fitness variance through the generations. 




Here, we consider each $i$ to refer
to an $L_i$-basepair segment under negative selection (though this model
accommodates other forms of polygenic selection, \cite[p.
2109]{Santiago1998-bs}). The inflation factor $Q_i^2$ depends on the
recombination fraction $r_i$ between the neutral site and selected segment, and
the rate at which the fitness variance at segment $i$ associated with the

neutral allele decays across generations, $1-\kappa_i$. This associated fitness
variation decays at rate $1-\kappa_i$ due only to the dynamics of polygenic
selection within the segment \parencite{Bulmer1980-zo,Turelli1990-kd}, assuming
a constant environment. Then, the root of the asymptotic variance inflation
factor $Q_i$ experienced by the neutral allele is,
...
variation $C_i^2$ can be specified and the change in variance due to selection
can be approximated as a geometric decay, i.e. $\Delta C_i^2 = -\kappa C_i^2$
\parencite{Bulmer1971-ae,Keightley1988-eq,Walsh2018-bt}. This is usually a
reasonable assumption since within-generation selection removes a fraction of
phenotypic variation from the population, and the fraction of that phenotypic
variation that is genetic is approximately the heritability $h^2$.

Assuming the fitness
variation $C_i^2$ in the segment is at an equilibrium between mutation and
selection, $\Delta C_i^2 = 0$, and $\Delta C_i^2 = V_{m,i} - \kappa C_i^2$


Note that
the decay in fitness variation due to the substitution of weakly deleterious
mutations happens on a slower timescale and is ignored here, but as we will
impacts the overall level of $C_i^2$. 
