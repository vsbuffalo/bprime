...
\begin{align}
      N_{e,\infty} &\approx N \exp\left(- \frac{V_{h}}{2n} \sum_{l=1}^n Q_{\infty,l}^2\right) \\
              &\approx N \exp\left(- \frac{V_{h}}{L} \int_0^{\nicefrac{L}{2}} Q_{\infty}(r)^2 dr \right) \\
\end{align}

where $L = n r_\text{BP}$ is the total segment length in Morgans. Then, the
total asymptotic effect of associations from all sites as $Q_\infty^2$.  Then,

\begin{align}
  Q_\infty^2 &= \frac{2}{L}\int_0^{\nicefrac{L}{2}} Q_{\infty}(r)^2 dr  \\
  &=  \frac{2}{L} \int_0^{\nicefrac{L}{2}} \left(\frac{1}{1 - (1-r)Z}\right)^2 dr  \\
  &= \frac{2}{(1-Z) (2-(2-L) Z)}
\end{align}
...
\begin{align}
  Q_\infty^2 = \frac{2}{L (1-Z)} + \frac{2 (L-2)}{L^2}+\frac{2 \left(L^2-4 L+4\right) (1-Z)}{L^3} + \mathcal{O}(\kappa^2)
\end{align}

Santiago and Caballero's approximation only keeps this first term, e.g. their
$Q_\infty^2 \approx 2/(L(1-Z))$, which is only accurate in the domain $L >
0.2$. 

\begin{align}
  N_{e,\infty}^{\text(SC98)} \approx N \exp\left(-\frac{V_h}{(1-Z)L}\right) 
\end{align}

(c.f. \cite{Santiago1998-bs} equation 8). For small $L$, the small $\kappa$
approximation strongly deviates from the full equation; thus, throughout, we
use the unapproximated version,

\begin{align}
  N_{e,\infty} \approx N \exp\left(-\frac{V_h}{(1-Z) (2-(2-L) Z)} \right).
\end{align}
...
\begin{align}
  ...
  N_{e,t} \approx N \exp\left(-\frac{V_h}{L} \int_0^{\nicefrac{L}{2}} \left(1+\frac{(1-k) (1-r) \left(1-(1-k)^t (1-r)^t\right)}{k+(1-k)r} \right) dr \right).
\end{align}
...
\begin{align}
  N_{e,\infty}^\text{BGS} = N \exp\left( - \frac{U}{2s + L - Ls} \right)
\end{align}
...
\textcite{Higgs1995-xc} (though they assume an asexual system, the result is
the same). In their work, they derive equations for the change per generation
for the moments and cross-moments of the fitness distribution under
multiplicative selection in a haploid population. For a Poisson number of new
mutations per gamete, per generation, they find (equation 13.3,
\cite{Higgs1995-xc}) that the rate of the ratchet is 

\begin{align}
  R = U - s \V(n).
\end{align}

The rate of the ratchet for a diploid population is the same, as long as $U$ is
the mutation rate per generation per diploid genome (as is assumed here).
$\V(n)$ is the variance of the distribution of the \emph{number} of deleterious
mutations (not the fitness distribution); as long as selection is not too
strong, $V_A \approx -s^2\V(n)$. Then, the rate of the ratchet in terms of the
fitness variance is,

\begin{align}
  %R &= U - s\V(n) \\
  R &\approx U + \frac{V_A}{s} \\
  V_A &\approx Us - Rs. 
\end{align}

This is identical to their equation 2, noting that the waiting time between
fixations (i.e. ratchet clicks) is $T = \nicefrac{1}{R}$. Note that if selected
sites are sufficiently deleterious that they cannot fix (or the population is
infinite), $R=0$ and the additive fitness variance is identical to that under
strong background selection, $V_A = Us = V_\text{BGS}$ (equation
\eqref{eq:va_bgs}).

Then, the fixation probability of a new deleterious mutation with additive
effects ($1-s$ in the heterozygote, $1-2s$ in the homozygote) in a diploid
population is,
...
inverse of the averaging waiting time until a fixation, $T \approx 1/(2UN
p_\text{fix})$, so

\begin{align}
  T \approx \frac{\exp(4 N_e s)-1}{4 U s N_e}.
\end{align}
...
\begin{align}
  R(N_e) := \frac{4 U s N_e}{\exp(4 N_e s)-1}
\end{align}
...
asymptotic reduction in effective population size is given by the following
system of non-linear equations,

\begin{align}
  R(N_{e,\infty}) &= \frac{4 U s N_{e,\infty}}{\exp(4 N_{e,\infty} s)-1}\\
  N_{e,\infty} &= N \exp\left(-\frac{(U-R(N_{e,\infty}))s}{\left(1-\frac{Us}{U-R(N_{e,\infty})}\right) \left(2-(2-L) \frac{Us}{U-R(N_{e,\infty})}\right)} \right).
\end{align}

An analogous system can be found for $N_{e,t}$ using equation
\eqref{eq:net_integral}.
