...
Overall, across our CADD-based models and all populations, our mutation rate
estimates are roughly concordant with pedigree-based estimates (Figure
\ref{fig:figure-4}A). Our full-track CADD 6\% model estimates a mutation rate
of $\mu = 1.46 \times 10^{-8}$ for YRI, $1.53 \times 10^{-8}$ for CEU, and $1.5
\times 10^{-8}$ for CHB reference samples (Supplementary Table
\ref{supp:tbl-r2}). Mutation rate estimates for our feature-based annotation
models are unrealistically higher. We believe this could be caused by
per-basepair heterogeneity in selective effects within each of these features.
For example, mutations in coding sequences are known to have highly
heterogeneous effects depending on whether the result is synonymous,
non-synonymous, or mis-sense. If some fraction of the impact of negative
selection is due to strong selection at a few sites, sparser annotation models
like these cannot isolate these effects like CADD-based models. Instead, XXXHERE 

reflects a general problem
with these more interpretable, but sparser feature-based tracks: heterogeneity
in selective effects 


However, we noticed that estimates of mutation rate is very sensitive to the
choice of annotation model. Between CADD 6\% and 6\% models, the 
