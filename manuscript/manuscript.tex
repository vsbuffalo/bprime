...
populations in or PhastCons Priority model (Supplementary Material
\ref{supp:locally-rescaled-fits}). When we re-ran optimization with an upper
limit of $\mu = 1 \times 10^{-7}$, the mutation rate was again estimated to
this (implausibly large) upper limit, with no considerable changes to model fit
or DFE estimates. Second, all of our leave-one-chromosome-out $R_\text{LOCO}^2$
are about one percentage point higher than the unrescaled model. Third, the DFE
estimates for PhastCons regions is now U-shaped (Supplementary Materials Figure
...
unfeasible. However, two observations indicate our findings here are robust
despite this limitation. First, the results do not change based on whether the
$B(x)$ is averaged at the 1 kbp level or at the megabase scale; for the latter,
a single segment makes little contribution to the average. Second, we
investigated the extent to which local-rescaling differed from the B' maps
across selection parameters. We find minor differences between the
locally-rescaled and standard B' maps in the nearly-neutral domain ($0.2 \le
2N_e s \le 2$); the maps were identical under strong selection ($2N_e s= 20$)
as expected (Supplementary Materials Figure \ref{suppfig:rescale-b}). Moreover,
the correlations between the standard and local-rescaled B' maps across the
genome are high (100\% for $2N_e s = 20$, 96.5\% for $2N_e s = 2$, and 60.21\%
for $2N_e s = 0.2$). The overall realized effect of local-rescaling is to just
...
sensitive the theoretic B' values in the $2Ns \le 1$ domain. The fact that
...
likelihood estimate of mutation is implausibly high.
