...
population through time under selection through the $N_{e,t}$ terms.  In the
original derivation of \textcite{Santiago1995-hx}, they set each $N_{e,t}$ term
to a single $N$ and solve this summation in the limit to infinity giving,
...
$V_h$ and $Z$ can be worked out. Other authors have extended similar models to
...
contributions from each locus $l$, $w_{i,l}$, giving $w_i = \prod_{l=1}^L
w_{i,l}$. Then, assuming mean fitness is one, the total fitness variation is
multiplicative across the locus-level fitnesses,

\begin{align}
  V_h %&= \E_i\left( w_i^2 \right) - 1 \\
      %&= \prod_{l=1}^L (1 + \alpha_{l}^2 \V(g_l))  - 1  \\
      &= \prod_{l=1}^L (1 + V_{h,l})  - 1
\end{align}

Now, the $Q_t^2 V_h$ term in the single-locus model is now under a
multiplicative polygenic model,

\begin{align}
  Q_t^2 V_h &= \prod_{l=1}^L \left(1 + \frac{V_{h,l} Q_{t,l}^2}{2}\right)  - 1
\end{align}
...
\begin{align}
  N_{e,t} &\approx \frac{N}{V_h Q_t^2 + 1} \\
          &\approx \frac{N}{\prod_{l=1}^L \left(1 + \frac{V_{h,l} Q_{t,l}^2}{2}\right)} \\
          &\approx N \exp\left(- \sum_{l=1}^L \frac{V_{h,l} Q_{t,l}^2}{2}\right)
\end{align}

(c.f. \cite{Santiago1998-bs} equation 4b). This summation can then in turn be
approximated by a integral; see \textcite{Santiago1998-bs} for more. Note that
another way to arrive at this equation is to take the product of reduction
factors $\nicefrac{N_e}{N}$ across selected sites, as in equation 4 of
\textcite{Hudson1995-xc}.
...
complicated due to the complexity of multilocus selection. However, several
quantitative genetic models simplify these dynamics by assuming an
infinitesimal model, where the trait is sufficiently polygenic that the allele
frequency changes at the underlying loci due to selection are negligible
compared to drift. This allows for a critical assumption of the quantitative
genetic models of linked selection, that the decay rate in variation is
constant (and thus independent of the underlying allele frequency changes). In
the Santiago and Caballero model, this is the decay factor $Z$.



The theory of \textcite{Santiago1998-bs} is based on an equilibrium model,
where $\Delta V_h = 0$ of the long run, even if the fitness variation
\emph{associated} with the allele's haplotype is changing. This macro-scale

\begin{align}
  \Delta V_h &= V_m - V_h\left(1 - \frac{1}{2 N_e}\right) \\
\end{align}
...
model, such that the fitness of an individual $i$ is

\begin{align}
  w_i = (1-s)^{n_i}
\end{align}

where $n_i$ is the number of deleterious mutations this individual carries.
...
expected number of mutations is $n_i = \nicefrac{\mu}{s}L$ where $L$ is the
total number of basepairs that can be mutated (the mutational target size).
Under a multiplicative fitness function in an infinite population, selection
cannot generate linkage disequilibria \parencite{Turelli1990-kd}. Thus the
classic strong background selection model assumes additive genetic fitness is
equal to the additive genic fitness (which excludes the contribution of
covariance between selected sites), 

\begin{align}
  \V_i(w_i) &= 2 \sum_{l=1}^L p(1-p) s^2 \\
            &= 2 L \frac{\mu}{s} \left( 1 - \frac{\mu}{s}\right) s^2 = U s + \mathcal{O}(\mu^2) \approx Us.
\end{align}

where we define $U := 2 L \mu$ to be the deleterious mutation rate per diploid
genome, per generation. 



Next, \textcite{Santiago1998-bs} find a general
equation for the factor $Z$ by which fitness variation associated with the
neutral allele is reduced under mutation-selection balance. This is a
macro-scale variable reflecting the average balance between the average loss of
fitness variance due to selection under an infinitesimal model and drift. Each
generation, the gain of new fitness variance per generation is $V_m$.
...
Then, with under the strong background selection model,
a site under purifying selection reduces diversity at a focal neutral site $r$
by a factor of

\begin{align}
  \frac{N_{e,t}}{N} = \exp \left(-\frac{V_{h,1} Q_{t,1}^2}{2}\right)
\end{align}
