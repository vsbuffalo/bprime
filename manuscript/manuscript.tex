...
\subsection*{Estimates of the Distribution of Fitness Effects in Humans}
...
likely conserved features are consistently estimated to have strongly
...
7.8\% chance, and $s=10^{-3}$ with 41\% chance. These estimates suggest a
bimodal DFE (Figure \ref{fig:figure-3}A, second row, left column), and are
mirrored across the other CADD models. 

Given that previous methods have typically not estimated the DFE up to
selection coefficients of $s=10^{-1}$, we ran additional models using a DFE
constrained to $s=10^{-2}$. Each of these constrained-DFE models across all
populations fit the data less well than models run with the grid up to
$s=10^{-1}$, explaining 1\% less of the leave-one-chromosome-out variation on
average. As expected, the constrained-DFE models put more mass on $s=10^{-2}$
(see Supplementary Table \ref{supp:tbl-r2}). Overall, this common behavior
across models seems likely to reflect a non-identifiability issue between
selective effects of $s=10^{-1}$ and $s=10^{-2}$, which we refer to as the
...
hypothetical limit, each occurrence of a very harmful mutation is like an
instantaneous death and thus acts like a genome-wide reduced effective
population size. In our model, this genome-wide effective population size is
parameterized by $\pi_0$; thus, one line of evidence for this effect is a
correlation between estimates of $\pi_0$ and average selection $\bar{s}$.
Looking at the CADD 6\% models fit across the default and constrained DFE
grids, we find this is the case. Across every population, average selection was
about 7 times larger using the default grid, and $\pi_0$ was $5.6\%$ larger.
There was no similar consistent change in mutation rate estimates among
populations. In the CADD 6\% model, genome-wide average reduction factor
$\bar{B}$ was $\approx 6.1\%$ lower in the default versus constrained grid.
Overall, this suggests that the linked selection signal alone carries little
information that can differentiate very strong selection from a slightly
smaller drift-effective population size.

Despite the strong-selection non-identifiability issue, we still were curious
how stable these estimates are across samples from reference populations. In
particular, we focused on the Feature Priority model, which fits the data less
well but is more interpretable since its annotation is based on gene features.
Overall, estimates of the distribution of selection coefficients are stable
across populations and had narrow uncertainty as estimated by a block jackknife
procedure (Figure \ref{figure-3}B). Coding sequences show the strong-selection
non-identifiability issue, and an uncertain signal of a neutral mode in the
DFE. This would be expected given the very different levels of selective
constraint synonymous and non-synonymous sites in coding sequences are under.
The PhastCons class of this Feature Priority model, which contains highly
conserved non-coding elements (XXX), is under strong selective constraint as
expected. Interestingly, in our PhastCons Priority annotation model (Figure
\ref{fig:figure-3}A bottom row), the DFE estimate for coding sequences
excluding PhastCons regions is neutral. This is expected; the selection signal
in coding regions is absorbed by the PhastCons feature.
...
Early genome-wide models of background selection closely fit megabase-scale
patterns in sequence divergence among five primates
\parencite{McVicker2009-ax}. However, the mutation rate estimates from this
model were unreasonably high compared to pedigree-based estimates (XXX). This
mutation rate discordance has led to the hypotheses that positive selection
could explain some fraction of the genome-wide linked selection signal (e.g.
\cite{Enard2014-kz}); however, evidence suggests that the role of classic
sweeps in humans is minuscule
\parencite{Pickrell2009-tt,Hernandez2011-gs,Murphy2022-sj}. Furthermore, Murphy
et al. find the classic BGS model fit using the CADD 6\% track leads to
mutation rate estimates that are consistent with pedigree-based rates. Given
that our method is based on a different theoretic approach, we curious whether
it would lead to estimates consistent with pedigree-based mutation rates.

Across all populations, our mutation rate estimates from CADD-based models are
roughly concordant with pedigree-based estimates (Figure \ref{fig:figure-4}A).
Our full-track CADD 6\% model estimates the mutation rate as $\widehat{\mu} =
1.46 \times 10^{-8}$ for YRI, $1.53 \times 10^{-8}$ for CEU, and $1.5 \times
...
several issues. First, mutation rate estimates under using our implementation
of classic BGS theory are an order of magnitude below the expected range
(Figure \ref{fig:figure-4}A top row). We observe similar behavior when we use
the classic BGS model to fit full-coverage annotation models (Figure
\ref{fig:figure-4}A bottom row). This behavior is consistent with the fact that
classic BGS theory cannot fit weak selection, and to accommodate features under
weak constrain (e.g. introns, UTRs, and the ``other" feature) it must severally
underestimate the mutation rate. 
...
these models is our best-fitting model, and the other our worst indicates a
large role in per-basepair heterogeneity in selective effects in determining
the genome-wide linked selection signal. CADD-based models fit well in part due
to their fine-scale resolution of selective effects across the genome.
...
non-identifiability issue, we next sought to assess the estimation accuracy of
our composite likelihood method using forward simulations. In order to have
enough megabase-scale data to apply our method, we independently evolved the
first five human chromosomes. Due to the immense computational expense of
conducting forward simulations at this scale over numerous parameter
combinations, we used fixed mutation and selection parameters in putatively
conserved regions (see Methods XXX). We note three important findings from
these simulations.
...
\ref{fig:figure-4}B, top row). This is consistent across both classic BGS and
our B' method; the only exception is the classic BGS model run with a
full-coverage track for the reasons described above. However, if new mutations
are highly deleterious ($s=0.05$), $R^2$ is reduced. This is consistent with
very strong selection having less localized effects and acting more like a
genome-wide faster rate of drift.
...
erroneously estimated strong selection and a very low mutation rate. By
...
coefficients. When BGS is not acting, either due to weak selection or a low
rate of deleterious mutations (Figure \ref{fig:figure-4}B right column), all
estimates are worse. However, we note that this is an unlikely region of
parameter space and this issue is readily diagnosable from the poor model fit 
as evident by low $R^2$ values.
...
population expansion would bias our method. We expansion rates like those
estimated for Europeans by \textcite{Gutenkunst2009-pg} had little effect on
parameter estimates (Supplementary Section \ref{supp:sim-assum}). By contrast,
we find our estimates are downwardly biased when mutational effects are weak
and recessive, but are accurate under recessive strong selection. This occurs
since the fixation probabilities of recessive mutations are in reality much
lower than they are under our model assuming additivity.
...
\parencite{Gilbert2020-aw}.  For model XXX 
