...
\textcite{Santiago2016-mu}; they define the sum the heterozygosity effective
population size.



Throughout, we assume that there is a constant \emph{drift-effective}
population size, $N_d$, which determines the rate of coalescence due only to
neutral demographic processes. We then consider the observed levels of neutral
pairwise diversity as a reduction factor down from the drift effective
population size, e.g. $\pi \approx 4 N_e \mu = 4 B N_d \mu$, where $B$ is a
reduction factor due to the background selection process.
