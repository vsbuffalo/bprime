...
\ref{eq:va_bgs}) and thus $\kappa_i^{BGS} = s$. Combining this with $\kappa_i =
\nicefrac{V_{m,i}}{V_{a,i}} = s$ in Equation \eqref{eq:simple_Ne}, we have

\begin{align}
    N_e = N \exp \left( - \sum_i^n \frac{\mu L_i}{s(1 + r_i(1-s)/s)^2} \right) 
\end{align}

which is exactly the genome-wide model of background selection used in previous
studies \parencite{McVicker2009-ax,Elyashiv2016-vt,Murphy2022-sj}. Thus, the
classic background selection model is a special case of the more general theory
of Santiago and Caballero (\citeyear{Santiago2016-mu}), which they have shown
previously (\citeyear{Santiago1998-bs}).
...
represent twice the rate of the ratchet of deleterious mutations in fixations
per generation, each fixation removes exactly the variation mutation puts in
$V_m = Us$ (we omit the segment index for clarity). Thus, under a general
equilibrium negative selection process,
...
Note that since mutation is the ultimate source of all mutations that fix or
are lost, $U \ge R$. Intuitively, this equilibrium genic fitness variation is a
balance between the rate of gain of genic fitness variation due to mutation and
its loss due to fixation. Our results differ from \parencite{Santiago2016-mu},
since we find from simulations their expression models the additive
\emph{genic} fitness variation, not additive genetic fitness variation (see
Section XXX), since it does not adequately capture linkage disequilibria
dynamics. Santiago and Caballero demonstrate Equation \eqref{eq:Va} through
Fisher's Fundamental Theorem of Natural Selection, but we find an alternative
proof (\cite{Higgs1995-xc}; see Supplementary Materials Section
\ref{supp:weak-strong}).

However, closed-form expressions for the rate of the ratchet $R$ have generally
been hard to find \parencite{Haigh1978-gt,Higgs1995-xc,Gessler1995-hz}. The key
insight of \parencite{Santiago2016-mu} is that the ratchet rate under draft is
given by the probability of fixation $p_f(N_e, s)$
\parencite{Kimura1962-su,Malecot1948-zv} with the draft-rescaled effective
population size, e.g. $R = N U p_f(N_e, s)$. Given this equation for the
ratchet and Equation \eqref{eq:polygenic_Ne} for $N_e$ under draft, we have a
system of non-linear equations that can be solved numerically for $N_e$ and
$R$.

\subsection*{Simulations of the Diploid Model}
...
where $U = 2u$ and $R=2r$ is the diploid rate of the ratchet, which depends on
