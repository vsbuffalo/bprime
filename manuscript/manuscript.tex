...
fitness or are deleterious. 
Natural selection works to eliminate these deleterious mutations from the population,
thus we expect them to appear at low frequencies (Haldane?Muller?) within populations
and be less likely to fix between lineages. 
The genome itself is the product of over a billion years of evolutionary optimization,
so even though they are rare within populations, we expect that deleterious variants
should be the overwhelming majority of segregating variation in functional regions,
and be responsible for a significant portion of phenotypic variation and disease (XX cites XX),

Selection on both beneficial and deleterious alleles leads to 
perturbations in the allele frequency spectrum of neighboring, linked variation.
This phenomenon, known as linked selection, is a major force shaping patterns of
genetic variation along chromosomes 
\parencite{Maynard_Smith1974-zr,Charlesworth1993-gb,Barton1998-aj,Gillespie2000-mh}.
As deleterious variation is clustered in functional portions of the genome,
we expect linked selection to reduce levels of diversity surrounding evolutionarily
conserved segments (e.g. genes, regulatory elements, etc.) 
and to shape large-scale spatial patterns in genetic diversity. 
This landscape of diversity is the product of a complex interplay between the
deleterious mutation rate, the local recombination rate, the strength of selection, and
the spatial distribution of conserved segments along a chromosome. 
Since genome-wide recombination maps and putatively conserved segments are available
for many species, there has been consistent effort to fit models of
linked selection to patterns of diversity to estimate population genetic
parameters such as the strength of selection and the deleterious mutation rate 
\parencite{Hudson1995-pt,McVicker2009-ax},
the overall reduction in genetic variability due to the
selection on deleterious variants \parencite{Elyashiv2016-vt,Murphy2022-sj},
and in some cases the rate of beneficial mutations \parencite{corbett2015natural,Elyashiv2016-vt}.

Prior attempts to model the reduction in linked diversity due to deleterious alleles
have largely relied on the now canonical Background Selection (BGS) model \parencite{Charlesworth1993-gb,Nordborg1996-nq}.
While this model has been successful in explaining many patterns of diversity,
some its simplifying assumptions
may distort inferences about the negative selection process.
First, since fixation probabilities ultimately
...
demography. Unfortunately, accommodating demography into analytic models of negative
selection remains an open, difficult problem (XXX). Second, building off
classic models of mutation-selection balance \parencite{Crow1970-wj,Kimura1966-bk}
BGS assumes that new mutations are sufficiently deleterious
such that they are invariably driven to loss. 
Under this assumption, the effect of selection is well-approximated by simply rescaling
the coalescent effective size by a reduction factor, 
sometimes called $B$ such that $N_eB=N_{0}$ \parencite{Charlesworth2013-kl}.
However this rescaling is not appropriate across parts of parameter space
that are relevant to natural populations segregating deleterious variation
 \parencite[e.g.][]{good2014genetic,Santiago2016-mu}.
In particular, because the BGS model cannot accommodate the
...
\parencite{santiago1998effective,Santiago2016-mu}.
...
Caballero (2016). 
This model is identical to BGS when selection against deleterious mutations is strong,
but it also correctly predicts the reduction in diversity and substitution rates of weakly
deleterious mutations by accurately modeling levels of genic fitness variation.
We extend Santiago and Caballero model (hereafter the SC16) so that it can be
used to model genome-wide patterns of diversity,
and develop Python software to use this theoretic approach to fit
genome-wide models of negative selection. Using forward simulations, we show
this model leads to more accurate DFE estimates under weak selection. 
Finally, we apply our composite-likelihood method to human population genomic data
and provide new estimates of the landscape of negative selection in humans.

%
%
% ADK- Vince I commented out the below because I don't like to walk through the results
% in an intro. Instead say what you did, and then the results are in the results section.
%
%While this new model accommodates the possibility that a given reduction B could be
...
that allows for extra non-heritable variability in offspring number $V_k$,

\begin{align}
    N_d = \frac{2N}{V_k + 2}
\end{align}

where if $V_k = 2$, this reduces to the classic Wright--Fisher model
...
increase the variance in allele frequency change. Unlike drift, the stochastic
perturbations each generation due to draft are autocorrelated as long as
segregation and recombination do not decouple an allele from its fitness
background and the selective environment does not change
\parencite{Robertson1961-ho,Santiago1995-hx,Buffalo2019-qs}. When selection is
strong, 

When selection is
weak and fitness is polygenic, the stochasticity introduced by draft is
well-approximated by rescaling the drift-effective population size 

that exist when there are heritable differences among haplotypes.
Mathematical models of genetic draft 

Gillespie2000-mh

,Robertson1961-ho,Santiago1995-hx,Santiago1998-bs,Barton2000-zg
...
%`'\cite{Garcia-Dorado2007-jj}). 
A more formal derivation follows from \textcite{Higgs1995-xc} 
(though they assume a haploid asexual system, the end
