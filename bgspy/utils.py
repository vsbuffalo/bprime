...
def si_prefix(quantity, digits=3):
    ...
    inverse_prefix = {int(np.log10(v)): p for p, v in prefix.items()}
    x = np.log10(quantity)
    if int(x) == x:
        x = int(x)  # simplify it
    prefix = inverse_prefix[int(np.ceil(np.log10(quantity)))]
    return f"{np.round(x, digits)}{prefix}"
